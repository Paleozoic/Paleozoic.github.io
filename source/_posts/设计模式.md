...
![设计模式_结构型模式_装饰者模式_UML图](/resources/img/design_patterns/设计模式_结构型模式_装饰者模式_UML图.png)
...
- Component：[IComponent](https://github.com/apache/storm/blob/master/storm-client/src/jvm/org/apache/storm/topology/IComponent.java)

- ConcreteComponent：[BaseBasicBolt](https://github.com/apache/storm/blob/master/storm-client/src/jvm/org/apache/storm/topology/base/BaseBasicBolt.java)

- Decorator：[Executor](https://github.com/apache/storm/blob/master/storm-client/src/jvm/org/apache/storm/executor/Executor.java)

- ConcreteDecorator：[BasicBoltExecutor](https://github.com/apache/storm/blob/master/storm-client/src/jvm/org/apache/storm/topology/BasicBoltExecutor.java) 将装饰者的属性`private IBasicBolt _bolt;`写在了具体装饰者里。`IBasicBolt `也是`IComponent`的子类。`BasicBoltExecutor`通过复写`public void execute(Tuple input)`方法，扩展了Bolt的功能。使得bolt execute之后会进行ACK。
...
`ProtocolFilterWrapper`扩展的功能如下：`export`和`refer`之前先判断是否是`Constants.REGISTRY_PROTOCOL`，执行不同的操作。
...
- Component：[Protocol](https://github.com/dangdangdotcom/dubbox/blob/master/dubbo-rpc/dubbo-rpc-api/src/main/java/com/alibaba/dubbo/rpc/Protocol.java)
- ConcreteComponent：[DubboProtocol](https://github.com/dangdangdotcom/dubbox/blob/master/dubbo-rpc/dubbo-rpc-default/src/main/java/com/alibaba/dubbo/rpc/protocol/dubbo/DubboProtocol.java)
- Decorator：[AbstractProtocol](https://github.com/dangdangdotcom/dubbox/blob/master/dubbo-rpc/dubbo-rpc-api/src/main/java/com/alibaba/dubbo/rpc/protocol/AbstractProtocol.java)
- ConcreteDecorator：[ProtocolFilterWrapper](https://github.com/dangdangdotcom/dubbox/blob/master/dubbo-rpc/dubbo-rpc-api/src/main/java/com/alibaba/dubbo/rpc/protocol/ProtocolFilterWrapper.java)
...
public class ConcreteDecorator implements Decorator{
  ...
  public ConcreteDecorator(Component component){//被装饰者通过参数传入，对客户端可见
    this.component = component;
  }
}
//代理
public class Proxy implements Subject{//被代理者通过在代理类内部构建，对客户端不可见
  private Subject subject;
  public Proxy(){
    this.subject = new RealSubject(); //一班大力类通过发射生成：Proxy.newProxyInstance
  }
}
```
...
![设计模式_结构型模式_代理模式_UML图](/resources/img/design_patterns/设计模式_结构型模式_代理模式_UML图.png)
...
- Subject：抽象观察目标
  ...
  * regist(Oberver)：被观察者将观察者注册，`notify()`将会通知这些观察者执行`doWhenNotify()`
  * unregist(Oberver)：将指定Oberver从订阅者列表移除
  * notify()：遍历observerList，然后调用`doWhenNotify()`

- ConcreteSubject：抽象观察目标

  * state：被观察者状态，该状态如果改变。则应调用`notify()`通知所有订阅者（观察者）。

![设计模式_行为型模式_责观察者模式_UML图](/resources/img/design_patterns/设计模式_行为型模式_观察者模式_UML图.png)

例子：[dubbo的RegistryDirectory](https://github.com/dangdangdotcom/dubbox/blob/master/dubbo-registry/dubbo-registry-api/src/main/java/com/alibaba/dubbo/registry/integration/RegistryDirectory.java)

dubbo注册中心的服务发布订阅也是典型的观察者模式。消费者观察注册中心，若注册中心发生变化，则调用notify更新服务。

TODO：这里的代码较为复杂，暂时不分析了。

- Observer：无，`public final class URL implements Serializable`
- ConcreteObserver：[URL](https://github.com/dangdangdotcom/dubbox/blob/master/dubbo-common/src/main/java/com/alibaba/dubbo/common/URL.java) 消费者
  * doWhenNotify()：具体看`RegistryDirectory#public synchronized void notify(List<URL> urls)`里面的代码
- Subject：[AbstractDirectory](https://github.com/dangdangdotcom/dubbox/blob/master/dubbo-cluster/src/main/java/com/alibaba/dubbo/rpc/cluster/directory/AbstractDirectory.java)
- ConcreteSubject：[RegistryDirectory](https://github.com/dangdangdotcom/dubbox/blob/master/dubbo-registry/dubbo-registry-api/src/main/java/com/alibaba/dubbo/registry/integration/RegistryDirectory.java) 注册中心，被观察者
  * observerList：URLs
  * regist(Oberver)：`public void subscribe(URL url)`
  * unregist(Oberver)：`public void destroy()`，该方法内部调用`egistry.unsubscribe(getConsumerUrl(), this);`取消订阅。
  * notify()：`public synchronized void notify(List<URL> urls)`
