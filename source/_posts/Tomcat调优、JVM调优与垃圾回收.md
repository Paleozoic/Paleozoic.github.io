...
# JVM运行时数据区
![线程状态](/resources/img/jvm/JVM运行时数据区.png)
- 程序计数器
...
## 垃圾标识算法
- JVM
- **引用计数法**：Java堆中每个Object都有一个计数器，Object被引用时计数器+1，当Object被置为null或者离开作用域时计数器-1.计数器为0的Object会被标识为“垃圾”对象。但此方法无法解决对象间循环引用的问题。
```java
ObjA a = new ObjA();
ObjB b = new ObjB();
//相互引用（循环引用，构成环）
...
//a，b的对象计数器都不为0
```
- **可达性分析算法**：JVM以一系列GCRoots为根节点，从GCRoot向下搜索，搜索的路径称之为引用链。不在引用链上的对象（无法搜索到的对象）便是“垃圾”对象。
...
- 年轻代(Young Generation)：年轻代分为3个区
  ...
  * To Survivor区：Young GC时，复制Eden区，From Survivor区存活的对象至To Survivor区。之后From/To互换。
  ...
  ![线程状态](/resources/img/jvm/年轻代.png)
- 老年代(Old Generation)：
...
[神奇的G1——Java全新垃圾回收机制](http://geek.csdn.net/news/detail/92144)
